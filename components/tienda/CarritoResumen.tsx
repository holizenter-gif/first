...
export default function CarritoResumen({ mostrarBotonPago = true }: CarritoResumenProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <h3 className="font-semibold text-brand-dark mb-4">Resumen de compra</h3>
      ...
      <div className="border-t pt-4 flex justify-between font-bold">
        <span>Total</span>
        <span className="text-brand-dark">${subtotal().toLocaleString()} MXN</span>
      </div>
      {mostrarBotonPago && (
        <a href="/checkout" className="mt-4 block text-center py-3 bg-brand-teal text-white font-semibold rounded-lg hover:bg-amber-700 transition-colors">
          Pagar ahora
        </a>
      )}
    </div>
