...
export default function TiendaClientPage({ productos }: Props) {
  ...
  const categorias = useMemo(() => {
    ...
    for (const p of productos) {
      if (!seen.has(p.categoria)) {
        ...
        const meta = CATEGORIAS_LABELS[p.categoria];
        cats.push({
          slug:   p.categoria,
  ...
  const destacados = useMemo(
    () => productos.filter((p) => p.activo && p.destacado),
    [productos]
  );

  const membresiaSlug = useMemo(
    () => productos.find((p) => p.categoria === "membresia" && p.activo)?.slug ?? null,
    [productos]
  );
        ...
        <div className="max-w-6xl mx-auto">
          {categoriaActiva !== "todos" || busqueda ? (
            <h2 className="font-display font-bold text-xl mb-6" style={{ color: "#0D1A0F" }}>
              ...
              <span className="ml-2 text-sm font-normal text-gray-400">({filtrados.length})</span>
            </h2>
          ) : (
            <h2 className="font-display font-bold text-xl mb-6" style={{ color: "#0D1A0F" }}>
              Todo el catálogo
