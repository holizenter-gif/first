...
export default function PagoMethods({ onSelectMethod, totalMXN }: PagoMethodsProps) {
  return (
    <div className="space-y-3">
      <h3 className="font-semibold text-brand-dark mb-4">Método de pago</h3>
      {([
        ...
        <button key={method.id} onClick={() => onSelectMethod(method.id)} className="w-full flex items-center gap-4 p-4 border border-gray-200 rounded-xl hover:border-brand-teal transition-colors text-left">
          <span className="text-2xl">{method.icon}</span>
          <div>
            <p className="font-semibold text-sm text-brand-dark">{method.label}</p>
            <p className="text-xs text-gray-500">{method.desc}</p>
          </div>
        </button>
      ))}
      <p className="text-center text-sm text-gray-500 mt-4">Total a pagar: <strong className="text-brand-dark">${totalMXN.toLocaleString()} MXN</strong></p>
    </div>
  );
}
