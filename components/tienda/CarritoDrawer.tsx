    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative bg-white w-full max-w-md h-full flex flex-col shadow-xl">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="font-serif text-xl font-bold text-brand-green">Carrito ({totalItems()})</h2>
            ...
            items.map((item) => (
              <div key={item.producto.id} className="flex gap-4 items-start">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm text-brand-green line-clamp-2">{item.producto.nombre}</p>
                  <p className="text-brand-gold font-bold text-sm mt-1">${item.producto.precio.toLocaleString()} MXN</p>
                </div>
        ...
        {items.length > 0 && (
          <div className="p-6 border-t">
            <div className="flex justify-between mb-4">
              ...
              <span className="font-bold text-brand-green">${subtotal().toLocaleString()} MXN</span>
            </div>
            <a href="/checkout" className="block w-full text-center py-3 bg-brand-gold text-white font-semibold rounded-lg hover:bg-amber-700 transition-colors">
              Proceder al pago
            </a>
          </div>
        )}
